...
class GridMerger:
    ...
    def __init__(self, grid_map, threshold):
        """
    ...
    def calculate_label_histograms(self, data):
        """
    ...
    def calculate_histogram_difference(self, histogram1, histogram2):
        """
    ...
    def is_difference_within_threshold(self, histogram1, histogram2, total_samples):
        """
    ...
    def merge_grids(self, grid_id1, grid_id2, samples1, samples2):
        """
        ...
        Returns:
            tuple: The new combined grid ID.
            list: The combined list of sample IDs from both grids.
        """
        ...
        for dim1, dim2 in zip(grid_id1, grid_id2):
            if dim1 == dim2:
                new_grid_id.append(dim1)
            else:
                # Ensure both dim1 and dim2 are treated as tuples before merging
                dim1_tuple = (dim1,) if not isinstance(dim1, tuple) else dim1
                dim2_tuple = (dim2,) if not isinstance(dim2, tuple) else dim2
                new_grid_id.append(dim1_tuple + dim2_tuple)  # Merge tuples

        return tuple(new_grid_id), combined_samples

    def merge_until_threshold(self, data):
        """
        ...
        Returns:
            dict: The final mapping of samples to merged grids.
        """
...
if __name__ == "__main__":
    file_path = os.path.join(os.path.dirname(__file__), 'data_clean_3.xlsx')
    divider = 2
    ...
    for grid_id, samples in final_grid_map.items():
        histogram = histograms[grid_id]
        histogram_str = ', '.join([f'{label}: {count}' for label, count in histogram.items()])
        print(f"{grid_id} --- {samples} --- Histogram: {histogram_str}")
        print()
