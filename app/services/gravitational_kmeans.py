...
class GravitationalKMeans:
    ...
    def __init__(self, data, initial_clusters, enhancement_factor=0, max_iterations=100):
        """
        ...
        # Initialize centroids, weights, and dominant labels for clusters
            ...
            for i in range(len(self.features)):
                ...
                for j, centroid_label in enumerate(self.dominant_labels):
                    if label == centroid_label:
                        forces[j] *= (1 + self.enhancement_factor)
                    else:
                        forces[j] *= (1 - self.enhancement_factor)
        ...
        while remaining_samples_indices:
            ...
            for sample_index in remaining_samples_indices:
                for cluster_index, (centroid, weight, dominant_label) in enumerate(
                    ...
                    if self.labels[sample_index] == dominant_label:
                        force *= (1 + self.enhancement_factor)
                    else:
                        force *= (1 - self.enhancement_factor)
            ...
            if sample_counter % 3 == 0 or not remaining_samples_indices:
    ...
    def evaluate_accuracy(self, clusters):
        """
    ...
    def count_labels_in_clusters(self, clusters):
        """
    ...
    def train_and_evaluate_nn(self, clusters, flag=0):
        """
        ...
        X_train, X_test, y_train, y_test = train_test_split(clustered_features, clustered_labels, test_size=0.2, random_state=42)
        ...
        if flag == 1:
            model.save('ML_model_for_stroke_prediction.h5')

        return accuracy * 100

    def train_and_evaluate_nn_2(self):
        """
        Trains and evaluates a neural network on the full dataset.
        """
    ...
    def train_and_evaluate_rf(self, clusters, flag=0):
        """
        ...
        if flag == 1:
            joblib.dump(rf_model, 'RF_model_for_stroke_prediction.pkl')

        return accuracy * 100

    def train_and_evaluate_xgb(self, clusters, flag=0):
        """
        ...
        if flag == 1:
            xgb_model.save_model('XGB_model_for_stroke_prediction.json')
...
if __name__ == "__main__":
    ...
    gkm = GravitationalKMeans(pd.read_excel(file_path), initial_clusters)
    gkm.fit_incremental()

    print("*" * 100)
    gkm.train_and_evaluate_nn_2()
